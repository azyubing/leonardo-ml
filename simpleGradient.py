...
@click.command()
@click.option('--threshold', default=0.0001, type=click.FLOAT,             # 学习结果接近的真实值的程度
...
@click.option('--point', nargs=2,
              type=click.Tuple([float, float]),  # click.FloatRange(0, 10, clamp=True)]),
...
def train(threshold, lr, initial_weight, initial_bias, point):
    ...
    while err > threshold:
        ...
        for x, y, y_true in zip(X, Y, Y_true):
            z = w * x + b
            gradient_w += (y - y_true) * sigmoid_derivation(z) * x
            gradient_b += (y - y_true) * sigmoid_derivation(z)
